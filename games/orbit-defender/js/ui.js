/**
 * ui.js
 * Komponen UI retro Kaplay untuk Orbit Defender: buttons, HP bars, effects, floating texts.
 */
...
export function makeHpBar(k, pos, maxHp = 100, width = 200, height = 16) {
  const bg = k.add([
  ...
  return {
    setValue(curHp) {
      ...
      if (pct > 0.5) {
        fill.color = k.rgb(34, 197, 94); // Hijau
      } else if (pct > 0.25) {
        fill.color = k.rgb(245, 158, 11); // Kuning
      } else {
        fill.color = k.rgb(239, 68, 68); // Merah bahaya
      }
    },
    destroy() {
      bg.destroy();
